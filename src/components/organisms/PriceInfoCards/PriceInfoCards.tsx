    ...
    <div className="grid w-full gap-y-4 sm:grid-cols-3 sm:gap-x-3 sm:gap-y-2 md:w-auto md:gap-x-[18px] md:gap-y-6">
      {data.map((data) => (
        ...
        >
          <div
            key={data.title}
            className="w-full rounded-lg border-2 border-orange-400 p-6 transition-all duration-200 ease-in-out hover:border-orange-500 hover:shadow-md md:min-w-[272px]"
          >
            <PriceInfo
              image={data.image}
              title={data.title}
